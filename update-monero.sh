...

{
	...
	USE_DEVICE_TREZOR=OFF USE_SINGLE_BUILDDIR=1 make -j$(nproc --ignore=2) && cp build/release/bin/monero* /usr/bin/ && chmod a+x /usr/bin/monero* &&	putvar "versions.monero" "$RELEASE"
} 2>&1 | tee -a "$DEBUG_LOG"
